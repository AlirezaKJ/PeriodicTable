function range(start, stop, step) { // python range function in js
...
// Set theme and save to local storage and button
if (darktheme === null) {
    darktheme = "true"
...
if (darktheme == "true") {
    ...
    cssr.style.setProperty('--rightbartransbg', 'rgba(0,0,0,0.5)');
    ...
    cssr.style.setProperty('--rightbartransbg', 'rgba(255,255,255,0.5)');
...
function addcell(elindex = "0") {
    if (elindex == 93) {
        ...
        cell.setAttribute("onclick", `openFBlock(0)`)
        cell.innerHTML = `
        <span class="fblockopenername">57-71</span>
        `
        cell.classList.add("element")
        cell.classList.add("item")
        cell.classList.add("fblockopener")
        cell.classList.add("row6")
        cell.classList.add("column3")
        cell.setAttribute("id", "57-71")
        tablediv.appendChild(cell)
    } else if (elindex == 111) {
        var cell = document.createElement("div")
        cell.setAttribute("onclick", `openFBlock(0)`)
        cell.innerHTML = `
        <span class="fblockopenername">89-103</span>
        `
        cell.classList.add("element")
        cell.classList.add("item")
        cell.classList.add("fblockopener")
        ...
        elementwmdb = wmdb[element.atomicnum - 1]
        var cell = document.createElement("div")
        ...
        // Add element groups classes
        elementmemberships = formatQuote(formatQuote(elementwmdb['"Memberships"'])).split(", ")
        elementmemberships.forEach(membership => {
            cell.classList.add(membership)
        })
...
// FBlock Functions
let fblockdiv = document.getElementById("fblock")
function openFBlock(state) { 
    if (state == 0) {
        fblockdiv.classList.remove("hidefblock")
    } else if (state == 1) {
        fblockdiv.classList.add("hidefblock")
        console.log("FBlock Open Req") 
    } else {
        fblockdiv.classList.toggle("hidefblock")
    }
}


function initFBlock(row) {
    rowdomelements = document.querySelectorAll(`#fblock #ftable .row${row}`)
    if (row == 1) {
        for (i in range(57, 72)) {
            i = parseInt(i)
            i = i + 57
            element = wmdb[i - 1];
            domelementdiv = rowdomelements[i - 57]
            elementabbreviation = formatQuote(element['"Abbreviation"'])
            elementatomicnumber = formatQuote(element['"AtomicNumber"'])
            elementstandardname = formatQuote(element['"StandardName"'])
            elementmemberships = formatQuote(formatQuote(element['"Memberships"']))
            elementmemberships = elementmemberships.split(", ")
            elementmemberships.forEach(membership => {
                domelementdiv.classList.add(membership)
            });
            domelementdiv.classList.add(elementstandardname)
            domelementdiv.setAttribute("onmouseover", `elementHover(${elementatomicnumber})`)
            domelementdiv.setAttribute("onclick", `elementClick(${elementatomicnumber})`)
            domelementdiv.setAttribute("id", elementstandardname)
            domelementdiv.innerHTML = `<div class="elname">${elementabbreviation}</div><div class="elatomicnum">${elementatomicnumber}</div>`
        }
    } else if (row == 2) {
        for (i in range(89, 104)) {
            ...
            i = i + 89
            element = wmdb[i - 1];
            domelementdiv = rowdomelements[i - 89]
            elementabbreviation = formatQuote(element['"Abbreviation"'])
            elementatomicnumber = formatQuote(element['"AtomicNumber"'])
            elementstandardname = formatQuote(element['"StandardName"'])
            elementmemberships = formatQuote(formatQuote(element['"Memberships"']))
...
let alltableelements = document.querySelectorAll("#table .element, #ftable .fitem")
...
function elementClick(atomicnum) {
    ...
    elementdiv = document.querySelector(`#${elStandardName}`)
    ...
    if (elementdiv.classList.contains("clicktgled")) {
        ...
        alltableelements.forEach(element => {
            ...
            element.classList.remove("clicktglLanthanum")
            element.classList.remove("clicktglLutetium")
            element.classList.remove("clicktglActinium")
            element.classList.remove("clicktglLawrencium")
            element.classList.remove("clicktglLanthanide")
            element.classList.remove("clicktglActinide")
        ...
        } else if (elementdiv.classList.contains("Lanthanum")) {
            elementdiv.classList.add("clicktglLanthanum")
        } else if (elementdiv.classList.contains("Lutetium")) {
            elementdiv.classList.add("clicktglLutetium")
        } else if (elementdiv.classList.contains("Actinium")) {
            elementdiv.classList.add("clicktglActinium")
        } else if (elementdiv.classList.contains("Lawrencium")) {
            elementdiv.classList.add("clicktglLawrencium")
        } else if (elementdiv.classList.contains("Lanthanide")) {
            elementdiv.classList.add("clicktglLanthanide")
        } else if (elementdiv.classList.contains("Actinide")) {
            elementdiv.classList.add("clicktglActinide")
        }
    }
}
...
// electron config bar and functions
...
function showElectronConfig(atomicnum) {
    ...
    elementElectronShellConfigStr = formatQuote(elementinfo['"ElectronConfigurationString"']).split("Superscript").join("").split(", ").join("").split("]").join("]  ")
...
let sidemenudiv = document.querySelector(".rightbar")